...
model = dict(
    ...
    pts_bbox_head=dict(
        type='MapTRv2Head_mask_F1_only_mask_DN_CP',
        bev_h=bev_h_,
        bev_w=bev_w_,
        num_query=900,
        num_vec_one2one=50,
        ...
        # z_cfg=z_cfg,
        transformer=dict(
            type='MapTRTransformer_longrange_1P',
            ...
            bev_h = bev_h_, 
            bev_w = bev_w_,
            patch_size = [bev_w_, bev_h_],
            # TODO dn ##
            num_vec_one2one=50,
            num_vec_one2many=0,
            
            
            with_cp_deform_attn = with_cp_deform_attn,
            
            embed_dims=_dim_,
            bev_encoder=None,
            bev_neck=dict(
                type="MSDeformAttnPixelDecoder_CP",
                num_outs=4,
...
dataset_type = 'CustomNuScenesOfflineLocalMapDataset_Longrange'
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes_map_infos_temporal_train_longrange.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes_map_infos_temporal_val_longrange.pkl',
        map_ann_file=data_root + 'nuscenes_map_anns_val_longrange.json',
...
lr_config = dict(
    ...
    min_lr_ratio=1e-3)
total_epochs = 24
evaluation = dict(interval=250, pipeline=test_pipeline, metric='chamfer',
