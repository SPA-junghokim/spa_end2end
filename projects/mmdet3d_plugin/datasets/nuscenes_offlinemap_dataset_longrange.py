...
class LiDARInstanceLines(object):
    ...
    def __init__(self, 
        ...
        self.mask_size = canvas_size  # TODO simple bev
    ...
    def Class_segemnts(self):
        ...
        for idx, instance in enumerate(self.instance_list):
            instance_label = self.instance_labels[idx]
            if instance_label == 0:
                self.line_ego_to_mask(instance, gt_semantic_mask_0[0], color=1, thickness=2)
            elif instance_label == 1:
                self.line_ego_to_mask(instance, gt_semantic_mask_1[0], color=1, thickness=2)
            elif instance_label == 2:
                self.line_ego_to_mask(instance, gt_semantic_mask_2[0], color=1, thickness=2)
    ...
    def instance_segments(self):
        ...
        for instance in instance_points_list:
            ...
            try:
                self.line_ego_to_mask(LineString(instance), instance_segm, color=1, thickness=2)
            except:
                pass
...
class CustomNuScenesOfflineLocalMapDataset_Longrange(CustomNuScenesDataset):
    r"""NuScenes Dataset.
            ...
            for k, v in info['annotation'].items():
                ...
                for instance in v:
                    ...
                    if self.MAPCLASSES.index(k) == 0:
                        self.line_ego_to_mask(line, gt_semantic_mask_0[0], color=1, thickness=2)
                    elif self.MAPCLASSES.index(k) == 1:
                        self.line_ego_to_mask(line, gt_semantic_mask_1[0], color=1, thickness=2)
                    elif self.MAPCLASSES.index(k) == 2:
                        self.line_ego_to_mask(line, gt_semantic_mask_2[0], color=1, thickness=2)
